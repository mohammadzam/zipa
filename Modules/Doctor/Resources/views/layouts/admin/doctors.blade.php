        ...
        <div class="container-fluid">
            ...
            <div class="row">
                        ...
                        <div class="row">
                            <div class="col">
                                ...
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active">{{trans('admin::admin.doctors index')}}</li>
                                </ol>
                            </div>
                            <!--end col-->
                ...
                <div class="col-sm-12 text-left mb-4">
                    <a  href="{{route('admin.create.new.doctor')}}" class="btn btn-primary px-4">{{trans('admin::admin.create_new_doctor')}}</a>
                </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped mb-0">
                                    ...
                                    <tr class="text-center">
                                        ...
                                        <th>{{trans('admin::admin.doctor_number')}}</th>
                                        <th>{{trans('admin::admin.status')}}</th>
                                        <th>{{trans('admin::admin.price_of_visit')}}</th>
                                        <th>{{trans('admin::admin.section_work')}}</th>
                                    ...
                                    <tr class="text-center">
                                        <td>{{$datum->id}}</td>
                                        <td>{{$datum->name}} </td>
                                        <td>{{$datum->doctor_number}}  </td>
                                        <td >
                                            <span class="{{$datum->status}}" >{{trans('admin::admin.'.$datum->status)}}</span>

                                        </td>
                                        <td>{{number_format($datum->costs->price)}} ريال</td>
                                        <td>{{$datum->sectionDoctor->section->name}} </td>
                                        <td>{{verta($datum->created_at)->format('Y/n/j')}}</td>
                                        <td class="d-flex justify-content-around align-items-center" style="height: 47px">
                                            <a href="{{route('admin.show.edit.form.doctor',$datum->id)}}">
                                                <i class="las la-pen text-info font-18"></i>
                                            </a>
                                            <a href="{{route('admin.getDoctorTimes',$datum->id)}}">
                                                <i class="las la-clock text-success font-18"></i>
                                            </a>
                                            <a href="{{route('admin.destroy.doctor',$datum->id)}}">
