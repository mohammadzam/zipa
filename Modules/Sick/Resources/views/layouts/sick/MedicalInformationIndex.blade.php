    ...
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            <div class="col-sm-12 text-left mb-4">
                <a  href="{{route('sick.create.new.medical.file')}}" class="btn btn-primary px-4">{{trans('admin::admin.new medical file')}}</a>
            </div>
        </div>
        <!--end row-->
                                ...
                                @if(isset($data)& $data->count()>0)
                                    @foreach($data as $datum)
                                        <tr class="text-center">
                                            ...
                                            <td>{{$datum->doctor->name??'هنوز تعيين نشده'}} </td>
                                            <td >
                                            ...
                                            <td>
                                                ...
                                                @else
                                                    <a class="text-danger" href="{{route('sick.pay.price.of.medical.operation',$datum->id)}}">
                                            ...
                                            <td class="d-flex justify-content-around align-items-center" style="height: 47px">
                                                <a href="{{route('sick.show.edit.medical.file.form',$datum->id)}}">
                                                    <i class="las la-pen text-info font-18"></i>
                                                </a>
                                                <a href="{{route('sick.show.my.medical.information.invoice',$datum->id)}}">
                                                    <i class="las la-clipboard-list text-success font-18"></i>
                                                </a>
{{--                                                <a href="{{route('sick.destroy.medical.file',$datum->id)}}">--}}
...
{{--                                                </a>--}}
                                ...
                                @else
                                    <td colspan="9">
                                        <h3 class="text-center text-danger">{{'پرونده پزشکی هنوز ثبت نشده، در صورت تمايل مى توانيد از دكمه ايجاد پرونده پزشکی استفاده كنيد'}}</h3>
                                    </td>
                                @endif
                                </tbody>
                            </table>
